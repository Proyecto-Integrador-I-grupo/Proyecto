...
-- Horario académico detallado de cada grupo.
-- El grupo conserva sus días y jornada general; esta tabla distribuye
-- las materias dentro de esa jornada (por ejemplo 08:00-09:00 Español,
-- 09:00-10:00 Matemáticas, etc.).
CREATE TABLE grupo_horario_academico(
    ...
    CONSTRAINT ck_grupo_horario_academico_horas CHECK (hora_fin > hora_inicio),
    CONSTRAINT fk_grupo_horario_academico_grupo
        FOREIGN KEY (id_grupo) REFERENCES grupo(id_grupo) ON DELETE CASCADE
);
CREATE INDEX idx_gha_grupo ON grupo_horario_academico(id_grupo);
CREATE INDEX idx_gha_dia_hora ON grupo_horario_academico(dia_semana, hora_inicio, hora_fin);
CREATE INDEX idx_gha_materia ON grupo_horario_academico(materia);
...
CREATE TABLE configuracion_integracion_servicios(
...
CREATE TABLE responsable_pago(
    ...
    estado BOOLEAN DEFAULT TRUE,
    principal_activo_estudiante INT GENERATED ALWAYS AS (
        CASE WHEN principal = TRUE AND estado = TRUE THEN id_estudiante ELSE NULL END
    ) STORED,
    CONSTRAINT fk_responsable_estudiante FOREIGN KEY (id_estudiante) REFERENCES estudiante(id_estudiante),
    CONSTRAINT uq_responsable_principal_activo UNIQUE (principal_activo_estudiante)
);
CREATE INDEX idx_responsable_estudiante ON responsable_pago(id_estudiante, principal, estado);
CREATE INDEX idx_responsable_identificacion ON responsable_pago(numero_identificacion, estado);
CREATE INDEX idx_responsable_correo ON responsable_pago(correo, estado);
...
CREATE TABLE documento_facturacion_integrada(
    ...
    CONSTRAINT uq_documento_integrado_cargo_tipo UNIQUE (id_cargo, tipo),
    CONSTRAINT fk_documento_integrado_cargo FOREIGN KEY (id_cargo) REFERENCES cargo_estudiante(id_cargo)
);
CREATE INDEX idx_documento_integrado_estado ON documento_facturacion_integrada(estado);

CREATE TABLE intento_pago_banco(
    ...
    CONSTRAINT uq_intento_pago_banco_token UNIQUE (token),
    CONSTRAINT uq_intento_pago_banco_referencia UNIQUE (referencia),
    CONSTRAINT uq_intento_pago_banco_transaction UNIQUE (transaction_code),
    CONSTRAINT uq_intento_pago_banco_payment UNIQUE (payment_id),
    CONSTRAINT fk_intento_pago_banco_cargo FOREIGN KEY (id_cargo) REFERENCES cargo_estudiante(id_cargo),
    CONSTRAINT fk_intento_pago_banco_usuario FOREIGN KEY (id_usuario) REFERENCES usuario(id_usuario)
);
CREATE INDEX idx_intento_pago_banco_cargo ON intento_pago_banco(id_cargo);
CREATE INDEX idx_intento_pago_banco_estado ON intento_pago_banco(estado);
...
-- Configuración inicial de endpoints. Las claves, credenciales y el merchant se
-- guardan después desde la interfaz; no se incluyen secretos dentro del SQL.
INSERT INTO configuracion_integracion_servicios
    (id_configuracion, factura_bonita_url, factura_bonita_api_key,
     banco_checkout_url, banco_login_url, banco_registro_url,
     banco_merchant_id, banco_afiliado, firma_digital_url,
     factura_electronica_url, factura_electronica_correo,
     factura_electronica_telefono, factura_electronica_password,
     factura_electronica_cuenta_confirmada, tributacion_url)
VALUES
    (1, NULL, NULL,
     'https://bankyfinanzas.netlify.app/checkout', NULL, NULL,
     NULL, FALSE, NULL,
     'https://proyecto-facturaci-n-electr-nica.onrender.com',
     'facturacion@educontrol.com', NULL, NULL, FALSE, NULL);
