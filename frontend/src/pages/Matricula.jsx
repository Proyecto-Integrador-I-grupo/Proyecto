    ...
    <section id="matricula-view" className="view hidden">
      ...
      <Modal id="modalGrupo" title={<><i className="bi bi-people"></i> Crear Grupo</>} lg className="edu-work-modal">
        <form id="grupo-form"><div className="modal-body p-4"><div className="row g-3 group-create-grid">
          <div className="col-md-6"><label className="form-label">Nombre del grupo</label><input id="grupo-nombre" className="form-control" maxLength="80" placeholder="Ej.: Grupo 6-A" required /></div>
          ...
          <div className="col-12"><label className="form-label schedule-label"><i className="bi bi-calendar-week schedule-field-icon"></i>Días de clase</label><div className="schedule-days schedule-days-week" id="grupo-dias">{['lunes','martes','miercoles','jueves','viernes'].map((dia) => <label key={dia} className="schedule-day"><input type="checkbox" value={dia} /> <span>{dia === 'miercoles' ? 'Miércoles' : dia.charAt(0).toUpperCase()+dia.slice(1)}</span></label>)}</div><div className="form-text">Selecciona los días en que funciona el grupo. Después podrás distribuir las materias por bloques dentro de la jornada semanal.</div></div>
          <div className="col-md-4"><label className="form-label schedule-label"><i className="bi bi-clock schedule-field-icon"></i>Inicio de jornada</label><input id="grupo-hora-inicio" type="time" className="form-control schedule-time-control" required /></div>
          <div className="col-md-4"><label className="form-label schedule-label"><i className="bi bi-clock-fill schedule-field-icon"></i>Fin de jornada</label><input id="grupo-hora-fin" type="time" className="form-control schedule-time-control" required /></div>
      ...
      <Modal id="modalHorarioGrupo" title={<><i className="bi bi-calendar3-week"></i> Horario semanal del grupo</>} lg className="edu-work-modal">
        <div className="modal-body p-4">
          <div className="row g-3">
            <div className="col-12"><label className="form-label">Grupo</label><select id="horario-grupo-select" className="form-select"><option value="">Seleccionar grupo</option></select><div className="form-text">Cada grupo puede recibir las 8 materias durante la semana. Los profesores se asignan después según su materia.</div></div>
            <div className="col-md-3"><label className="form-label">Día</label><select id="horario-grupo-dia" className="form-select"><option value="lunes">Lunes</option><option value="martes">Martes</option><option value="miercoles">Miércoles</option><option value="jueves">Jueves</option><option value="viernes">Viernes</option></select></div>
            <div className="col-md-3"><label className="form-label">Inicio</label><input id="horario-grupo-inicio" type="time" className="form-control" /></div>
            <div className="col-md-3"><label className="form-label">Fin</label><input id="horario-grupo-fin" type="time" className="form-control" /></div>
            <div className="col-md-3"><label className="form-label">Materia</label><select id="horario-grupo-materia" className="form-select"><option>Español</option><option>Matemáticas</option><option>Ciencias</option><option>Estudios Sociales</option><option>Inglés</option><option>Educación Física</option><option>Informática</option><option>Artes</option></select></div>
            <div className="col-12 d-flex justify-content-end"><button type="button" id="horario-grupo-agregar" className="btn btn-outline-primary"><i className="bi bi-plus-lg me-1"></i>Agregar bloque</button></div>
            <div className="col-12"><div id="horario-grupo-resumen" className="weekly-schedule-editor"></div></div>
          </div>
        </div>
        <div className="modal-footer"><button type="button" className="btn btn-outline-secondary" data-bs-dismiss="modal">Cerrar</button><button type="button" id="horario-grupo-guardar" className="btn btn-primary"><i className="bi bi-check2-circle me-1"></i>Guardar horario</button></div>
      </Modal>
