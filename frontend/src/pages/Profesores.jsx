...
export default function Profesores() {
  ...
  return (
    <section id="profesores-view" className="view hidden">
      ...
      <Modal id="modalEliminarProfesor" title={<><i className="bi bi-trash"></i> Eliminar Profesor</>}><div className="modal-body p-4"><p className="mb-2">¿Deseas eliminar a <strong id="eliminar-nombre-profesor"></strong> del cuerpo docente?</p><p className="small text-muted mb-0">Dejará de aparecer en la lista y perderá el acceso al sistema. El historial académico ya registrado se conserva para reportes y auditoría.</p></div><div className="modal-footer"><button type="button" className="btn btn-outline-secondary" data-bs-dismiss="modal">Cancelar</button><button type="button" id="confirmar-eliminar-btn" className="btn btn-danger edu-danger-solid"><i className="bi bi-trash3 me-1"></i>Eliminar</button></div></Modal>
      ...
      <Modal id="modalAsignarGrupos" title={<><i className="bi bi-diagram-3"></i> Asignar Grupos</>}><div className="modal-body p-4"><p>Profesor: <strong id="asignar-grupos-nombre-profesor"></strong></p><p className="text-muted small mb-1">Materia: <span id="asignar-grupos-materia"></span></p><p className="form-text mt-0 mb-3">El sistema valida automáticamente que el docente no quede asignado a dos grupos con clases superpuestas en el mismo período lectivo.</p><div id="asignar-grupos-error" className="alert alert-danger py-2 px-3 small d-none" role="alert"></div><input id="asignar-grupos-search" className="form-control form-control-sm mb-3" placeholder="Buscar grupo..." /><div id="asignar-grupos-lista" className="d-grid gap-2"></div></div><div className="modal-footer"><button type="button" className="btn btn-outline-secondary" data-bs-dismiss="modal">Cancelar</button><button type="button" id="confirmar-asignar-grupos-btn" className="btn btn-primary">Guardar asignación</button></div></Modal>
      <Horarios />
    </section>
  );
}
